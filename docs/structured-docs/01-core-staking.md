...
VaultHub → Lido.burnExternalShares(shares)                // allowed while staking PAUSED, blocked while STOPPED
  ...
  → if limit set and not paused: prevStakeLimit += stethAmount   // restores limit; does NOT add ETH to buffer
