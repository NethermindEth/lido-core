---
doc: "core-invariants"
title: Invariants - core 
contracts: []
prereqs: ["AGENTS-1"]
see_also: ["01", "02", "03", "04", "09"]
ssot_for: []
---
...
```jsonc
{
  ...
  "condition": "<precise property, with file:line refs to enforcement sites>",
  "path": "<contract path>"
}
```
...
Read these first to anchor on properties that are known and intentional. Then look at source for anything the file doesn't cover. condition text uses symbol names (line numbers are illustrative); resolve against live source.

---

# Sections

1. Core staking & tokens — Lido, StETH, StakeLimitUtils, NodeOperatorsRegistry.
2. Accounting / Burn & report execution — Accounting, Burner, OracleReportSanityChecker.
...
[
  ...
  },
  {
    ...
    "function": "Contract-wide",
    "condition": "The sum of shares[a] across every account a equals _getTotalShares() at all times. Every mutation flows through one of three internal functions: _mintShares (StETH.sol:518–535), _burnShares (:547–557), or _transferShares (:494–505), and each updates both shares[a] and totalShares symmetrically. This is the core accounting identity the share rate prices against. If it breaks, every balanceOf, every withdrawal claim, and every reward computation reads a stale denominator.",
...
## 2. Accounting/ Burn & report execution
```json
[
  {
  ...
  {
    "description": "prefinalize is pure and its returned ETH/shares match what _finalize would lock",
  ...
  },
  {
    ...
    "condition": "sharesToMintAsFees > 0 iff (report.clBalance + update.withdrawalsVaultTransfer) > update.principalClBalance. Per LIP-12, a flat or negative CL delta MUST produce sharesToMintAsFees == 0. Any path that mints fee shares on a non-positive CL delta is a violation.",
  ...
  },
  {
    ...
    "function": "_calculateTotalProtocolFeeShares",
    "condition": "When totalRewards = (report.clBalance + withdrawalsVaultTransfer) − principalClBalance + elRewardsVaultTransfer > 0: feeEther = (totalRewards × totalFee) / precisionPoints; sharesToMintAsFees = (feeEther × internalSharesBeforeFees) / (postInternalEther − feeEther). The formula makes the post-mint internal share rate equal to (postInternalEther − feeEther) / (internalSharesBeforeFees − sharesAlreadyBurned) — i.e. recipients receive shares worth exactly feeEther at the new rate. A different formula either over- or under-pays fee recipients relative to the rewards they're entitled to.",
    "path": "contracts/0.8.9/Accounting.sol"
  },
  {
