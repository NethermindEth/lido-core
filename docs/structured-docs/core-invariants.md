...
[
  ...
  },
  {
    ...
    "function": "Contract-wide",
    "condition": "Four pairings: (1) default payable / submit: mint shares to msg.sender AND increment bufferedEther by msg.value. (2) receiveELRewards (_auth(_elRewardsVault)) : increment TOTAL_EL_REWARDS_COLLECTED by msg.value (buffer credited later, inside collectRewardsAndProcessWithdrawals during the report). (3) receiveWithdrawals (_auth(_withdrawalVault)) : event-only at inflow (buffer credited inside the report). (4) rebalanceExternalEtherToInternal (_auth(_vaultHub())) : requires msg.value == getPooledEthBySharesRoundUp(shares); increments bufferedEther by msg.value AND decreases externalShares by the same shares (totalShares unchanged) — vault debt pay-down, see 01 flow 6. Bijection must hold both ways: every accounting-update path requires a matching msg.value of the same magnitude, and every ETH-receiving path through Lido's own code must trigger its paired update.",
    "path": "contracts/0.4.24/Lido.sol"
  },
  {
